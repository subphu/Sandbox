...
void Camera::move(glm::vec3 direction) {
    ...
    if (lockFocus && direction.z == 0)
        position = position * distance / getDistance();
}
...
void Camera::setInvertedAxis(bool value)    { axis = value ? -1 : 1; }
void Camera::setLockFocus   (bool value)    { lockFocus = value; }
void Camera::setSpeed       (float value)   { speed = value; }
void Camera::setPosition    (glm::vec3 pos) { position = pos; }

float     Camera::getDistance()   { return glm::distance(position, focusPoint); }
